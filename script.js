...
const drawerSections = {
  ...
  "shopping": {
    ...
    related: ["GRA Shop Demo", "Partnerangebote Demo", "Impressum", "Datenschutz"]
  }
};
...
const searchItems = [
  ...
  },
  {
    ...
    text: "Demo-Suche fuer Vereine, Sachkundetage und Informationsabende in deutschen Staedten.",
    image: "assets/member-card.svg",
...
function setupChat() {
  ...
  qs("#chatNow")?.addEventListener("click", () => {
    compact();
    showToast("Live-Chat Demo: Es wurde keine Verbindung aufgebaut.");
  });
  qs("#liveChatButton")?.addEventListener("click", () => {
    showToast("Live-Chat Demo: Nachrichten werden nicht gesendet.");
  });
}
...
function setupForms() {
  qs("#loginForm")?.addEventListener("submit", (event) => {
    event.preventDefault();
    showToast("Demo-Login: Zugangsdaten werden nicht geprueft oder gespeichert.");
  });
  ...
  qsa("[data-add-plan]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      showToast(`${plan} wurde in den Demo-Warenkorb gelegt.`);
  ...
  qs("#orderForm")?.addEventListener("submit", (event) => {
    event.preventDefault();
    showToast("Demo-Bestellung gespeichert. Es wurden keine Daten uebertragen und keine Zahlung ausgeloest.");
  });
}
...
function setupSearchResultsPage() {
  ...
  if (count) count.textContent = `${pretendCount.toLocaleString("de-DE")} Demo-Ergebnisse`;
...
function setupTableButtons() {
  qsa("[data-info]").forEach((button) => {
    button.addEventListener("click", () => {
      showToast("Demo-Detailseite: Termine sind beispielhaft und koennen nicht gebucht werden.");
    });
  });
}
